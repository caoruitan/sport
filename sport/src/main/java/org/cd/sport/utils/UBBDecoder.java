...
public class UBBDecoder {
	 ...
	 *            用户自定义的UBB标签的处理器的实例
	 * @param mode
	 *            容错模式，可以是忽略模式(MODE_IGNORE)或关闭模式(MODE_CLOSE)
	...
	public static final String decode(String s) {
		return UBBDecoder.decode(s, new SimpleTagHandler(), UBBDecoder.MODE_CLOSE);
	}
		...
		while (pos < len) { // 只要未到文件末尾就循环
			...
			if (cc[pos + 1] == '/') { // 标签以'/'起始，可能是结束标签
				...
				if (ss != null && ss.length == 3) { // 处理空标签
					// 先把当前文本扩展到当前节点
					addTextChild(node, buf);
					UBBNode nd = new UBBNode(node, ss[0].toLowerCase(), ss[1].split(","),
			...
			if (ss != null && ss.length == 2) { // 是合法标签
				...
				UBBNode nd = new UBBNode(node, ss[0].toLowerCase(), ss[1].split(","), new String(cc, pos, ii + 1 - pos),
						false);
				node.addChild(nd);
				pos = ii + 1;
				stk.push(nd);
...
class UBBNode {
	...
	public final String toString() {
		return "[tag=\"" + tag + "\",attr=\"" + attr + "\",closed=" + closed + ",children="
...
class SimpleTagHandler implements UBBTagHandler {
	...
	public String[] parseTag(String s, boolean isEmpty) {
		...
		// 只有下面的标记是本处理器支持的
		if ("b".equals(tmp) || "i".equals(tmp) || "u".equals(tmp) || "size".equals(tmp) || "color".equals(tmp)
				|| "quote".equals(tmp) || "url".equals(tmp) || "email".equals(tmp) || "img".equals(tmp)
				|| "*".equals(tmp) || "list".equals(tmp) || "fly".equals(tmp) || "move".equals(tmp)
				|| "align".equals(tmp) || "flash".equals(tmp) || "wmv".equals(tmp) || "rm".equals(tmp)
				|| "code".equals(tmp) || "glow".equals(tmp) || "sign".equals(tmp) || "em".equals(tmp)) {
		...
		if ("b".equals(tmp) || "i".equals(tmp) || "u".equals(tmp)) {
			...
			if (code.equals("#") == true) {
				...
				if (index < 0) {
					...
					if ((head.equals("http:") == false) && (head.equals("ftp:") == false)
							&& (head.equals("gopher:") == false) && (head.equals("news:") == false)
							&& (head.equals("telnet:") == false) && (head.equals("mms:") == false)
			...
			return "<OBJECT CLASSID=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" WIDTH=" + width + " HEIGHT=" + height
					...
					+ " PLAY=TRUE LOOP=TRUE QUALITY=HIGH></EMBED></OBJECT><br />[<a target=_blank href='" + data
					+ "'>全屏播放</a>]";
		} else if ("wmv".equals(tmp)) {
			return "<EMBED src='" + data + "' HEIGHT=\"256\" WIDTH=\"314\" AutoStart=1></EMBED>";
			...
			return "<img src='../ico/em" + attr[0] + ".gif' style=\"border:0\" >";
			...
			return "<span style='WIDTH:" + glowwidth + ";filter:glow(color=" + glowcolor + ", strength=" + glowstrength
					+ ")'>" + data + "</span>";
			...
			return "<div class=\"clearfix\">&nbsp;</div><div class=\"somecode\"><div class=\"somecode_main\"><div class=\"fheader_fillet\"><div class=\"fheader_fillet1\"><div class=\"fheader_fillet2\"></div></div></div><div class=\"body\"><div class=\"code_header\"><input type=\"button\" value=\"复制代码\" onclick=\"getcode('code-content"
					+ s + "')\" class=\"btn\"/></div><div class=\"code_tools\"><a class=\"c_to\" id=\"c_to" + s
					+ "\" onclick=\"showsomecode('" + s
					+ "')\">&nbsp;</a></div><div class=\"code-content\" id=\"code-content" + s + "\">" + data
					+ "</div><div id=\"code_footer" + s
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(
				"=========================\n" + UBBDecoder.decode(ubb, new SimpleTagHandler(), UBBDecoder.MODE_CLOSE));
	}
