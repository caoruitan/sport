			...
			<form id="sport-news-form">
				...
				<table class="editTable">
					...
					<tr>
						...
						<td>
							<textarea class="ckeditor" id="content">${news.content}</textarea>
							...
							<div class="news-file-container">
								<c:forEach items="${news.files}" var="file" varStatus="status">
									<div class="sport-news-file" data-index="${status.index}">
										<input type="hidden" name="files[${status.index}].id" value="${file.id}"/>
					            	 	<input type="hidden" name="files[${status.index}].name" value="${file.name}"/>
					            	 	<input type="hidden" name="files[${status.index}].path" value="${file.path}"/>
					            	 	<span style="text-align:left;display:inline-block;width:90%;line-height: 25px;color:#1e5494;">${file.name}</span>
