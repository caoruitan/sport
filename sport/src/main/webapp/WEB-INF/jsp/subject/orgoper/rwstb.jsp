...
<body>
	...
	<div class="titleBox1">
		...
		<div class="btnBox">
			...
			<c:if test='${not empty rws && rws.status ne "ORG_SP"}'>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#tj').dialog({
				...
				ok: function() {
					$.ajax({
						url: "<%=basePath%>/subject/orgoper/rwsPass.action",
						type: "POST",
						dataType: "JSON",
						data: {
							subjectId : "${subjectId}",
			...
			$('#th').dialog({
				...
				ok: function() {
					$.ajax({
						url: "<%=basePath%>/subject/orgoper/rwsUnpass.action",
						type: "POST",
						dataType: "JSON",
						data: {
							subjectId : "${subjectId}",
		...
		$("#e-jdap").click(function(){
			showLayout('b-jdap');
			$(".b-jdap").load("<%=basePath%>/subject/schedule/orgoper/list.htm?rwsId=${rws.rwsId}&subjectId=${subjectId}");
		});
		
		$("#e-sbgzmx").click(function(){
			showLayout('b-sbgzmx');
			$(".b-sbgzmx").load("<%=basePath%>/subject/device/orgoper/list.htm?rwsId=${rws.rwsId}&subjectId=${subjectId}");
		});
		
		$("#e-xbfjf").click(function(){
			showLayout('b-xbfjf');
			$(".b-xbfjf").load("<%=basePath%>/subject/appropriation/orgoper/list.htm?rwsId=${rws.rwsId}&subjectId=${subjectId}");
		});
		
		$("#e-jfys").click(function(){
			showLayout('b-jfys');
			$(".b-jfys").load("<%=basePath%>/subject/rwsbudget/orgoper/cost.htm?rwsId=${rws.rwsId}&subjectId=${subjectId}");
		});
	</script>
</body>
