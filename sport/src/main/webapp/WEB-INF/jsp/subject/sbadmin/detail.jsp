...
<body>
	...
	<div class="titleBox">
		...
		<a href="<%=basePath %>/subject/sbadmin/list.htm">
				...
				<div class="line">
						...
						<li class="cur">
							...
							<p>
								<c:if test='${not empty sbs}'>
									<a href="<%=basePath %>/subject/sbadmin/sbstb.shtm?subjectId=${subject.id}" target="_blank">
										<c:if test='${sbs.status ne "SBADMIN_SP"}'>
					...
					<c:if test='${subject.stage eq "RWSTB"}'>
						<li class="ok">
							<p><b>申报书填报与评审</b></p>
							<p>截止 <fmt:formatDate value="${subject.sbsEndDate}" type="date"/></p>
							<p><a href="<%=basePath %>/subject/sbadmin/sbstb.shtm?subjectId=${subject.id}" target="_blank"><button class="btn-red btn-size-small">查看</button></a></p>
						</li>
						<li class="cur">
							<p><b>任务书填报与评审</b></p>
							<p>截止 <fmt:formatDate value="${subject.rwsEndDate}" type="date"/></p>
							<p>
								<c:if test='${not empty rws}'>
									<a href="<%=basePath %>/subject/sbadmin/rwstb.shtm?subjectId=${subject.id}" target="_blank">
										<c:if test='${rws.status ne "SBADMIN_SP"}'>
											<button class="btn-red btn-size-small">查看</button>
										</c:if>
										<c:if test='${rws.status eq "SBADMIN_SP"}'>
											<button class="btn-red btn-size-small">审批</button>
										</c:if>
									</a>
								</c:if>
					...
					<c:if test='${subject.stage eq "JTBG"}'>
						<li class="ok">
							<p><b>申报书填报与评审</b></p>
							<p>截止 <fmt:formatDate value="${subject.sbsEndDate}" type="date"/></p>
							<p><a href="<%=basePath %>/subject/sbadmin/conclusiontb.shtm?subjectId=${subject.id}" target="_blank"><button class="btn-red btn-size-small">查看</button></a></p>
						</li>
						<li class="ok">
							<p><b>任务书填报与评审</b></p>
							<p>截止 <fmt:formatDate value="${subject.rwsEndDate}" type="date"/></p>
						...
						<li class="cur">
							...
							<p>
								<c:if test='${not empty sc}'>
									<a href="<%=basePath %>/subject/sbadmin/conclusiontb.shtm?subjectId=${subject.id}" target="_blank">
										<c:if test='${sc.status ne "SBADMIN_SP"}'>
											<button class="btn-red btn-size-small">查看</button>
										</c:if>
